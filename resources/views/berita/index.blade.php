...
<main>
    ...
    <h1 class="title">Table Berita</h1>
    <ul class="breadcrumbs">
        ...
        @else
            <div class="btn-group btn-group-sm d-flex justify-content-end" role="group" aria-label="Basic mixed styles example">
                <a type="button" href="{{ route('berita.create') }}" class="btn btn-primary"><i class='bx bxs-folder-plus'></i>Tambah Data</a>
                ...
                <table class="table table-striped table-responsive ">
                    ...
                    <tbody>
                        @foreach ($berita as $no => $brt)
                            <tr>
                                <input type="hidden" class="delete_id" value="{{ $brt->id }}">
                                <th>{{ $no+1 }}</th>
                                <td>{{ $brt->judul_berita }}</td>
                                <td>
                                    <img src="{{ asset('uploads/berita/'. $brt->cover_berita) }}" height="90" width="100" class="mt-3" >
                                </td>
                                <td>
                                    <img src="{{ asset('uploads/berita/'. $brt->gambar_berita) }}" height="90" width="100" class="mt-3" >
                                </td>
                                <td>
                                    {!! $brt->artikel_berita !!}
                                </td>
                                <td>
                                    ...
                                    @method('delete')
                                    <div class="btn-group">
                                        <a href="{{ route('berita.edit',$brt->id) }}" class="btn btn-warning btn-sm"><i class='bx bxs-edit'></i></a>
