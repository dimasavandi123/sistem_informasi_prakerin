    ...
    <div class="card mt-3">
        <div class="card-body">
            @if (session('success'))
                ...
                @endif
            <form action="{{ route('collaborate.update',$collaborate->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('patch')
            <div class="form-group mb-3">
                <label for="">Nama Industri</label>
                <input type="text"  name="nama_industri" value="{{ $collaborate->nama_industri }}">
            </div>
            <div class="form-group mb-3">
                <label for="">Logo Industri</label>
                <input type="file" name="logo_industri" >
                <img src="{{ asset('uploads/kerjasama/'. $collaborate->logo_industri) }}" height="90" width="100" class="mt-3" >
            </div>
            <div class="form-group mb-3">
                <label for="">Jurusan Sekolah</label>
                <input type="text"  name="jurusan_sekolah" value="{{ $collaborate->jurusan_sekolah }}">
            </div>
