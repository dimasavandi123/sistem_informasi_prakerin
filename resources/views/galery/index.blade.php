...
<main>
    ...
    <h1 class="title">Table Galery</h1>
    <ul class="breadcrumbs">
        ...
        @else
            <div class="btn-group btn-group-sm d-flex justify-content-end" role="group" aria-label="Basic mixed styles example">
                <a type="button" href="{{ route('galery.create') }}" class="btn btn-primary"><i class='bx bxs-folder-plus'></i>Tambah Data</a>
        ...
        @endif
            <div class="scrol mt-3" style="overflow-x: auto;">
                <table class="table table-striped table-responsive ">
                    ...
                    <tbody>
                        @foreach ($galery as $no => $gry)
                            <tr>
                                <input type="hidden" class="delete_id" value="{{ $gry->id }}">
                                <th>{{ $no+1 }}</th>
                                <td>{{ $gry->judul_galery }}</td>
                                <td>
                                    <img src="{{ asset('uploads/galery/'. $gry->gambar_galery) }}" height="90" width="100" class="mt-3" >
                                </td>
                       
                                <td>
                                    <form action="{{ route('galery.destroy',$gry->id) }}" method="POST">
                                    @csrf
                                    @method('delete')
                                    <div class="btn-group">
                                        <a href="{{ route('galery.edit',$gry->id) }}" class="btn btn-warning btn-sm"><i class='bx bxs-edit'></i></a>
