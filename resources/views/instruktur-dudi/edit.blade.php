    ...
    <div class="card mt-3">
        <div class="card-body">
            @if (session('success'))
                ...
                @endif
            <form action="{{ route('instruktur-dudi.update',$instruktur->id) }}" method="POST">
                @csrf
                @method('patch')
            <div class="form-group mb-3">
                <label for="">Nama Instruktur</label>
                <input type="text" placeholder="Pak Nugroho" name="nama_instruktur" value="{{ $instruktur->nama_instruktur }}">
            </div>
            <div class="form-group mb-3">
                <label for="">Tempat Dudi</label>
                <select name="prakerin_id" id="prakerin">
                        @foreach ($prakerin as $tempat)
                        <option value="{{ $tempat->id }}" {{ $instruktur->prakerin_id == $tempat->id ? 'selected' : '' }}>{{ $tempat->tempatPrakerin->nama_dudi }}</option>
                        @endforeach
                </select>
            </div>
            <div class="form-group mb-3">
                ...
                <select name="siswa_id" id="siswa">
                    @foreach ($siswa as $sw)
                     <option value="{{ $sw->id }}" {{ $instruktur->siswa_id == $sw->id ? 'selected' : '' }}>{{ $sw->nama_siswa }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group mb-3">
                <label for="">Kelas Siswa</label>
                <input type="text" name="kelas_jurusan_siswa" id="kelas_jurusan_siswa" readonly value="{{ $instruktur->kelas->kelas_jurusan_siswa }}">
            </div>
            <input type="hidden" name="kelas_id" readonly id="kelas_id" value="{{ $instruktur->kelas_id }}">
