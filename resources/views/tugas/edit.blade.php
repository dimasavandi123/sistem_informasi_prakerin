...
<main>
    <h1 class="title">Form Edit Tugas</h1>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('tugas.update', $tugas->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control" name="nama_tugas" value="{{ $tugas->nama_tugas }}">
                </div>
                <div class="form-group mb-3">
                    <label for="deskripsi">Deskripsi:</label>
                    <textarea name="deskripsi" class="form-control" id="deskripsi">{{ $tugas->deskripsi }}</textarea>
                </div>
                <div id="kolom-container">
                    @foreach ($tugas->kolomTugas as $index => $kolom)
                    <div class="kolom-group">
                        <span class="nomor-urut">{{ $loop->iteration }}.</span>
                        <label for="kolom_nama[]">Soal Tugas:</label>
                        <input type="text" name="kolom_nama[]" value="{{ $kolom->kolom_nama }}" required>
                        {{-- <label for="kolom_tipe[]">Tipe Kolom:</label>
                        <select name="kolom_tipe[]" required>
                            <option value="text" {{ $kolom->kolom_tipe == 'text' ? 'selected' : '' }}>Text</option>
                            <option value="number" {{ $kolom->kolom_tipe == 'number' ? 'selected' : '' }}>Number</option>
                            <option value="textarea" {{ $kolom->kolom_tipe == 'textarea' ? 'selected' : '' }}>Textarea</option>
                        </select> --}}
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="gurumapel_id" id="gurumapel_id" class="form-control">
                        ...
                        <option value="{{ $mapel->id }}" {{ $tugas->gurumapel_id == $mapel->id ? 'selected' : '' }}>
                            {{ $mapel->nama_mapel }}
                        </option>
                        @endforeach
                    </select>
                  ...
                  {{-- TUGAS KE --}}
                  <input type="hidden" name="tugas_ke" class="form-control" value="{{ $tugas->tugas_ke  }}" >
