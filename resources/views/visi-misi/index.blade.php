...
<main>
    ...
    <h1 class="title">Table Visi Misi</h1>
    <ul class="breadcrumbs">
        ...
        @else
            <div class="btn-group btn-group-sm d-flex justify-content-end" role="group" aria-label="Basic mixed styles example">
                <a type="button" href="{{ route('visimisi.create') }}" class="btn btn-primary"><i class='bx bxs-folder-plus'></i>Tambah Data</a>
            ...
            <div class="scrol mt-3" style="overflow-x: auto;">
                <table class="table table-striped table-responsive ">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Gambar Visi Misi</th>
                            <th>Judul Visi Misi</th>      
                            <th>Slogan Visi Misi</th>      
                            <th>Deskripsi Visi Misi</th>      
                            <th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($visimisi as $no => $vm)
                            <tr>
                                <input type="hidden" class="delete_id" value="{{ $vm->id }}">
                                <th>{{ $no+1 }}</th>
                                <td>
                                    <img src="{{ asset('uploads/visi-misi/'. $vm->gambar_visimisi) }}" height="90" width="100" class="mt-3" >
                                </td>
                                <td>{{ $vm->judul_visimisi }}</td>
                                <td>{{ $vm->slogan_visimisi }}</td>
                                <td>{{ $vm->deskripsi_visimisi }}</td>
                                <td>
                                    <form action="{{ route('visimisi.destroy',$vm->id) }}" method="POST">
                                    @csrf
                                    @method('delete') 
                                    <div class="btn-group">
                                        <a href="{{ route('visimisi.edit',$vm->id) }}" class="btn btn-warning btn-sm"><i class='bx bxs-edit'></i></a>
